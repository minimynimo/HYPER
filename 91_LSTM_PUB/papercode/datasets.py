...
from .datautils import (load_attributes, normalize_features,reshape_data, load_data)
...
class CamelsTXT(Dataset):
    ...
    def _load_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.file_num is None or self.varssim_dir is None or self.loc is None:
            raise ValueError("file_num, varssim_dir, and loc must be provided.")
        
        # Pass start_date and end_date to load_data
        df = load_data(self.file_num, self.varssim_dir,  self.loc)
        ...
        x = np.array([
            df['Precip'].values, df['Temp'].values, df['PET'].values
        ]).T

        y = np.array([df['Obs flow'].values]).T
    ...
    def _load_attributes(self) -> torch.Tensor:
        df = load_attributes(self.db_path, drop_lat_lon=False)
        ...
        # Convert self.basin to the same type as the DataFrame index
        try:
            basin_key = int(self.basin)  # Convert to int if possible
        except ValueError:
            basin_key = self.basin  # Keep as string if conversion fails

        # Check if the basin exists in the attributes
        if basin_key not in df.index:
            raise ValueError(f"No attributes found for basin {self.basin}. Check the attributes data.")
        ...
        # Extract attributes for the given basin
        attributes = df.loc[df.index == basin_key].values

        # Ensure attributes have valid dimensions
        if attributes.size == 0:
            raise ValueError(f"Attributes for basin {self.basin} are empty after processing.")
...
class CamelsH5(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if not self.no_static:
            ...
            if attributes.size == 0:
                raise ValueError(f"No attributes found for basin {basin}. Check the attributes data.")
    ...
    def _load_attributes(self):
        df = load_attributes(
            ...
            drop_lat_lon=False,
            keep_features=None  # Explicitly pass keep_features as None if not used
        )
        ...
        # Convert index to strings for consistent comparison
        df.index = df.index.map(str)

        # Check if the basin exists in the attributes
        if self.basins and not set(self.basins).issubset(df.index):
            print(self.basins)
            print(df.index)
            missing_basins = set(self.basins) - set(df.index)
            print(f"Warning: Missing attributes for basins: {missing_basins}")
            raise ValueError(f"Missing attributes for basins: {missing_basins}")
