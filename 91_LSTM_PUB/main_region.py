...
def get_args() -> Dict:
    ...
    parser.add_argument(
        'mode', 
        choices=["train", "evaluate", "eval_robustness", "create_splits", "create_region_train_test"],  # Added create_region_train_test
    )
    ...
    parser.add_argument(
        ...
        help="Training basins to select by region."
    )
...
def _setup_run(cfg: Dict) -> Dict:
    ...
    run_name = f'run_{cfg["region"]}_{day}{month}_{hour}{minute}_seed{cfg["seed"]}'
...
def train(cfg):
    ...
    print(f"Starting training with seed={cfg['seed']} and region={cfg['region']}")
    ...
    if cfg["split_file"] is not None:
        ...
        basins = splits.get("train", [])
        # Add check for empty train basins
        if not basins:
            print(f"ERROR: The loaded train basins list is empty!")
            print(f"  - Check if the split file was created with the same region ('{cfg['region']}') and seed ({cfg['seed']}) as the current run.")
            print(f"  - Loaded split file: {split_file_path}")
            print(f"  - Split file keys: {list(splits.keys())}")
            print(f"  - Current config: region={cfg['region']}, seed={cfg['seed']}")
            raise RuntimeError("Empty training basins list. Likely cause: region/seed mismatch between split file and current run.")
    else:
    ...
    try:
        ...
        print(f"[DEBUG] Region: {cfg['region']} | Seed: {cfg['seed']} | Number of training samples: {len(ds)}")
        if len(ds) == 0:
            raise RuntimeError(f"Training dataset is empty for region '{cfg['region']}' and seed {cfg['seed']}. Check your split file and region assignment.")
        loader = DataLoader(
...
fig
    ...
    if user_cfg.get("split_file") is not None:
        with Path(user_cfg["split_file"]).open('rb') as fp:
            splits = pickle.load(fp)
        basins = splits.get("test", [])
        basins = [str(basin) for basin in basins]
    else:
        # All possible basins as strings
        all_basins = [str(basin) for basin in file_num_list]
        # Training basins: all in region, not in test_basins_list
        train_basins = sorted(region_list_df[region_list_df['Region'] == f"{user_cfg['region']}"]['File_num'].tolist())
        train_basins = [str(file_num) for file_num in train_basins if str(file_num) not in [str(tb) for tb in test_basins_list]]
        # Test basins: all not in training basins
        basins = [basin for basin in all_basins if basin not in train_basins]
...
######################################
# Select region train and test basin #
######################################

def create_region_train_test(cfg: dict):
    """Create region train-test selection for evaluation.

    Takes basins test_basins_list and sets them as fixed test basins. The rest of the basins are used for region selection of training basins.
    A single region selection of training basins is created for each seed. The remaining basins are used as test basins.
    ...
    output_file = Path(f'{output_dir}/region_splits_{cfg["region"]}_seed{cfg["seed"]}.p') 
    ...
    train_basins = sorted(region_list_df[region_list_df['Region'] == f"{cfg['region']}"]['File_num'].tolist())
