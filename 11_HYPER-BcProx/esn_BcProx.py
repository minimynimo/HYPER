...
class ESN():
    ...
    def train(self, observed_data, target_data=None, washout=0, ridge_param=0.01, spinoff = False):
        ...
        #pbar = ProgressBar(0, train_length)
    ...
    def predict(self, current_reservoir, test_observed, ptb_func=None, ptb_scale=0.0, nexttime=False, extended_interval=0):
        ...
        #pbar = ProgressBar(0, test_length)

        # initial predict
        current_reservoir = self.forward(test_observed[:, 0] + ptb(), current_reservoir)    # (R, )
        ...
        #pbar.update(test_length)
        #pbar.finish()

        # predict (1,m)
        return predict
    
    def predict_PUB(self, W_out_weights, test_observed, ptb_func=None, ptb_scale=0.0, nexttime=False, extended_interval=0):
        """
        Generate the predict time series.
        args:
        - current_reservoir     : last reservoir state in training phase
        ...
        - extended_interval     : interval to give the true state
        """
        # W_out_weights <- W_out of donor basin    #(1,R)
        # test_observed <- input_eva        #(4,m)
