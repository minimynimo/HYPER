...
output_base_dir = f'/data0/funato/0_out/99_out/{loc}/BC_PUB_{reservoir_size}_{ridge_param}'
    ...
    for step_val in step_val_list:
        ...
        for file_num in train_basins: # go through only the training basins
            ...
            if file_num == train_basins[0]:
                W_out_df.to_csv(output_dir + f"/train_basin/Wout/BC_PUB_W_out{file_tag}.csv", mode='w', index=False, header=False)
            else:
                W_out_df.to_csv(output_dir + f"/train_basin/Wout/BC_PUB_W_out{file_tag}.csv", mode='a', index=False, header=False)
            """
            # W_out (1,R)
            # reservoir (R, )
