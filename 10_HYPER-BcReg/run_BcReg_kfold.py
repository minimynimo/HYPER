...
def PCA_lasso(pc_values, attribute_values, file_list_train, file_list_test, n_components, alpha, fs, output_dir, output_fig_dir, columns_drop, sample = False):
    ...
    # Ensure the 'File_num' column is not included in the PCA
    ...
    for pc_n in range(1, n_components+1):  # Skip the 'file_num' column
        ...
        lasso = Lasso(alpha=alpha, max_iter=1000000)  # Increase max_iter to address convergence warning
        ...
        predicted_pc_train = lasso_models[f'PC{pc_n}'].predict(attribute_values_train)
        predicted_pc_test = lasso_models[f'PC{pc_n}'].predict(attribute_values_test) 

        # revert the standardized values TODO

        predicted_pcs_train[f'PC{pc_n}'] =predicted_pc_train
        predicted_pcs_test[f'PC{pc_n}'] =predicted_pc_test
...
def run_BC_post_PCA(model, file_list, loc, output_dir, train_test, PC_n, weights_inverted, model_list, start_date_eva, end_date_eva, benchmark_list, nexttime, file_tag, varssim_dir, fold,log_file=None, sample = False):   
    result_eva_rev = []
    reservoir_rev_rows = []
...
def weight_vector(bma_weights_df, RC_weights_df, reservoir_size, model_list, file_list):
    ...
    # Set the 0 column as the index for RC_weights_df
    RC_weights_df.set_index(0, inplace=True)
